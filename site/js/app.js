...
myApp.controller('PortfolioListController', function( $scope ) {
	...
	$scope.addPortfolio = function (new_portfolio) {
		...
		if ( !new_portfolio.title || new_portfolio.title.length > 30 ) {
			$scope.add_portfolio_error = 'Please provide a title with less than 30 characters.';
		} else if ( !new_portfolio.date || new_portfolio.date.length < 10 ) {
			$scope.add_portfolio_error = 'Please provide a date in the format mm/dd/yyyy.';
		} else if ( !new_portfolio.name || new_portfolio.name.length < 6 ) {
			$scope.add_portfolio_error = 'Please provide a name with at least 6 characters.';
